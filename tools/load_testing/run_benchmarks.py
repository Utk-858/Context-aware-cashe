...
class ResourceMonitor(threading.Thread):
    ...
    def get_averages(self) -> Tuple[float, float, float, float]:
        ...
        proc_cpu = sum(self.proc_cpu_samples) / len(self.proc_cpu_samples) if self.proc_cpu_samples else 0.0
        proc_mem = sum(self.proc_mem_samples) / len(self.proc_mem_samples) if self.proc_mem_samples else 0.0
...
def start_server(workers: int, use_local_embeddings: bool = False) -> subprocess.Popen:
    ...
    print(f"Starting FastAPI server with workers={workers} (use_local_embeddings={use_local_embeddings})...")
    cmd = [
        sys.executable, "-m", "uvicorn", "tools.load_testing.server:app",
...
def compute_metrics_delta(baseline: Dict[str, Dict[str, float]], ending: Dict[str, Dict[str, float]]) -> Dict[str, float]:
    ...
    for pid in all_pids:
        ...
        delta["redis_sum"] += max(0.0, e_pid.get("rag_cache_redis_latency_seconds_sum", 0.0) - b_pid.get("rag_cache_redis_latency_seconds_sum", 0.0))
        delta["redis_count"] += max(0.0, e_pid.get("rag_cache_redis_latency_seconds_count", 0.0) - b_pid.get("rag_cache_redis_latency_seconds_count", 0.0))
        
        delta["faiss_sum"] += max(0.0, e_pid.get("rag_cache_faiss_search_latency_seconds_sum", 0.0) - b_pid.get("rag_cache_faiss_search_latency_seconds_sum", 0.0))
        delta["faiss_count"] += max(0.0, e_pid.get("rag_cache_faiss_search_latency_seconds_count", 0.0) - b_pid.get("rag_cache_faiss_search_latency_seconds_count", 0.0))
        
        delta["embedding_sum"] += max(0.0, e_pid.get("rag_cache_embedding_latency_seconds_sum", 0.0) - b_pid.get("rag_cache_embedding_latency_seconds_sum", 0.0))
        delta["embedding_count"] += max(0.0, e_pid.get("rag_cache_embedding_latency_seconds_count", 0.0) - b_pid.get("rag_cache_embedding_latency_seconds_count", 0.0))
        
    redis_avg = (delta["redis_sum"] / delta["redis_count"]) * 1000 if delta["redis_count"] > 0 else 0.0
    faiss_avg = (delta["faiss_sum"] / delta["faiss_count"]) * 1000 if delta["faiss_count"] > 0 else 0.0
    emb_avg = (delta["embedding_sum"] / delta["embedding_count"]) * 1000 if delta["embedding_count"] > 0 else 0.0
...
def run_single_benchmark(test_config: Dict[str, Any], workers: int, use_local_embeddings: bool = False) -> Dict[str, Any]:
    """Run a single benchmark configuration and return results."""
    print("\n" + "="*50)
    print(f"RUNNING: {test_config['name']} with workers={workers} (use_local_embeddings={use_local_embeddings}, Duration: {TEST_DURATION})")
    print("="*50)
...
def main():
    ...
    report.append(f"  - Redis Latency: Mock = **{final_multi['redis_ms']:.2f}ms**, Real = **{final_real['redis_ms']:.2f}ms**")
    report.append(f"  - FAISS Latency: Mock = **{final_multi['faiss_ms']:.2f}ms**, Real = **{final_real['faiss_ms']:.2f}ms**")
    report.append(f"  - Embedding Generation Latency: Mock = **{final_multi['embedding_ms']:.2f}ms**, Real = **{final_real['embedding_ms']:.2f}ms**")
    
    # 3. Queueing delay vs Service time
    mock_service_time = final_multi['redis_ms'] + final_multi['faiss_ms'] + final_multi['embedding_ms']
    ...
    report.append(f"  - Mock Embeddings: Service Time = **{mock_service_time:.2f}ms**, Client Latency (Avg) = **{f_sec(final_multi['avg_ms'])}**")
    report.append(f"  - Real Embeddings: Service Time = **{real_service_time:.2f}ms**, Client Latency (Avg) = **{f_sec(final_real['avg_ms'])}**")
