...
def run_benchmark_for_store(store_type, workload, embedding_cache, embedder):
    ...
    if store_type == "FAISS":
        ...
        if os.path.exists("faiss_index.bin"):
            try:
                ...
                pass
        vector_db = FaissVectorStore(dimension=384, kv_store=shared_kv, index_filepath="faiss_index.bin")
    else:
    ...
    avg_search_latency_ms = (sum(vector_search_latencies) / len(vector_search_latencies)) * 1000.0 if vector_search_latencies else 0.0
    ...
    if store_type == "FAISS":
        ...
        for item in vector_db.storage:
            memory_size += sys.getsizeof(item) + sys.getsizeof(item.get("id", "")) + sys.getsizeof(item.get("vector", []))
...
def main():
    ...
    print(f"{'Store Type':<12} | {'L1 Hit %':<8} | {'L2 Hit %':<8} | {'LLM Saved':<9} | {'Avg Lat':<7} | {'Avg Search':<10} | {'Memory (KB)':<11} | {'P50':<6} | {'P95':<6}")
    print("-" * 105)
    for r in all_results:
        print(f"{r['store_type']:<12} | {r['l1_hit_rate']*100:<7.1f}% | {r['l2_hit_rate']*100:<7.1f}% | {r['llm_calls_saved']:<9} | {r['avg_latency_ms']:<7.1f} | {r['avg_search_latency_ms']:<10.4f} | {r['memory_usage_kb']:<11.2f} | {r['p50_ms']:<6.1f} | {r['p95_ms']:<6.1f}")
    print("=================================================================")
    ...
    with open(csv_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow([
            "Store Type", "L1 Hit Rate", "L2 Hit Rate", 
            "Retriever Calls Saved", "LLM Calls Saved", 
            "Total Runtime (s)", "Avg Latency (ms)", "Avg Search Latency (ms)",
            "Memory Usage (KB)", "P50 (ms)", "P95 (ms)", "P99 (ms)"
        ])
        for r in all_results:
            writer.writerow([
                r["store_type"], f"{r['l1_hit_rate']:.4f}", f"{r['l2_hit_rate']:.4f}",
                r["retriever_calls_saved"], r["llm_calls_saved"],
                f"{r['total_runtime_sec']:.4f}", f"{r['avg_latency_ms']:.4f}", f"{r['avg_search_latency_ms']:.4f}",
                f"{r['memory_usage_kb']:.4f}", f"{r['p50_ms']:.4f}", f"{r['p95_ms']:.4f}", f"{r['p99_ms']:.4f}"
            ])
