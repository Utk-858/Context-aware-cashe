...
def verify_tenant_isolation():
    ...
    assert any(k.startswith("tenant:tenant_a:l1:") for k in all_keys), "L1 keys for tenant_a must be prefixed"
    assert any(k.startswith("tenant:tenant_b:l1:") for k in all_keys), "L1 keys for tenant_b must be prefixed"
    assert any(k.startswith("tenant:tenant_a:l2:") for k in all_keys), "L2 keys for tenant_a must be prefixed"
    assert any(k.startswith("tenant:tenant_b:l2:") for k in all_keys), "L2 keys for tenant_b must be prefixed"
    assert any(k.startswith("tenant:tenant_a:map:") for k in all_keys), "Map keys for tenant_a must be prefixed"
    assert any(k.startswith("tenant:tenant_b:map:") for k in all_keys), "Map keys for tenant_b must be prefixed"
