...
def run_diagnostics():
    ...
    print(f"{'Query':<36} | {'Total':<6} | {'Embed':<6} | {'Redis':<6} | {'VecSrc':<6} | {'DecEng':<6} | {'Retr':<6} | {'LLM':<6} | {'Result':<6}")
    print("-" * 105)
    ...
    for idx, q in enumerate(SAMPLE_QUERIES):
        ...
        print(f"{q_short:<36} | {total_time:.3f} | {embed_total:.3f} | {redis_total:.3f} | {vector_total:.3f} | {decision_total:.3f} | {retriever_time:.3f} | {llm_time:.3f} | {outcome:<6}")
