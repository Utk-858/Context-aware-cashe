...
L1_HITS = Counter(
    "rag_cache_l1_hits_total",
    "Total number of L1 cache hits",
    ["tenant_id"]
)
L1_MISSES = Counter(
    "rag_cache_l1_misses_total",
...
L2_HITS = Counter(
    "rag_cache_l2_hits_total",
    "Total number of L2 cache hits",
    ["tenant_id"]
)
L2_MISSES = Counter(
    "rag_cache_l2_misses_total",
...
CACHE_ENTRIES = Gauge(
    ...
    "Total number of keys/entries in Redis"
)

REDIS_MEMORY_BYTES = Gauge(
    "rag_cache_redis_memory_bytes",
    "Redis used memory in bytes"
)

FAISS_VECTORS_TOTAL = Gauge(
    "rag_cache_faiss_vectors_total",
