...
class FaissVectorStore(VectorStore):
    ...
    def upsert(self, ids: List[str], vectors: List[List[float]], metadata: List[Dict[str, Any]] = None, tenant_id: Optional[str] = None) -> None:
        """
        ...
        for i, cache_key in enumerate(ids):
            ...
            if mapped_data and "int_id" in mapped_data:
                ...
                self.kv.set(f"map:cache_key:{cache_key}", {"int_id": int_id}, tenant_id=active_tenant)
                query_str = metadata[i].get("query", "") if (metadata and i < len(metadata)) else ""
                self.kv.set(f"map:vector_id:{int_id}", {"cache_key": cache_key, "query": query_str, "tenant_id": active_tenant}, tenant_id=active_tenant)
    ...
    def search(self, query_vector: List[float], top_k: int = 5, tenant_id: Optional[str] = None) -> List[Dict[str, Any]]:
        """
